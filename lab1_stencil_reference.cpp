...
void proc_mid_naive(int xo, int yo) {
	int steps = g_status.blockwidth;
	float* ft = g_status.to + stepelems*(g_status.steps-1);
	//printf( "%d %d\n", xo, yo );

	for ( int j = yo; j < yo+steps; j++ ) {
		for ( int i = xo; i < xo+steps; i++ ) {
			stencil(i,j,g_status.from, ft, g_status.conduct);
			//if ( g_status.from[IDX(i,j)] > 0 ) printf( "%f!!\n", ft[IDX(i,j)] );
			//printf( "%d %d -- %d -- %f!!\n", i, j, IDX(i,j), ft[IDX(i,j)] );
//
		}
	}
	for ( int t = 1; t < g_status.steps; t++ ) {
...
void proc_mid_naive_avx(int xo, int yo) {
	int steps = g_status.steps;
	int bw = g_status.blockwidth;
	float* ft = g_status.to + stepelems*(steps-1);
	//printf( "%d %d\n", xo, yo );

	for ( int j = yo; j < yo+bw; j++ ) {
		for ( int i = xo; i < xo+bw; i+= 8 ) {
			stencil_avx(i,j,g_status.from, ft, g_status.conduct);
		}
	}
	for ( int t = 1; t < steps; t++ ) {
		float* ft = g_status.to + stepelems*(steps-1-t);
		float* ff = g_status.to + stepelems*(steps-t);
		//float* ftb = g_status.to + stepelems*((steps-1-t)%2);
		//float* ffb = g_status.to + stepelems*((steps-t)%2);
...
inline void proc_mid_single(int xo, int yo, int to, int w, int tt) {
	int steps = w;
	float* ft = g_status.to + stepelems*(g_status.steps-1);
	//printf( "%d %d %d -- %d %d\n", xo, yo, to, w, tt );
